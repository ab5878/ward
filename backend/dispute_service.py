...
"""
            ...
            timeline_cursor = self.db.timeline_events.find({"case_id": case_id}).sort("timestamp", 1)
            ...
            score = case.get("evidence_score", {})
            score_text = f"""EVIDENCE COMPLETENESS CERTIFICATE
---------------------------------
Score: {score.get('score', 0)}/100
Generated: {score.get('last_calculated')}

SECURED ITEMS:
{chr(10).join(['[x] ' + s for s in score.get('breakdown', [])])}

MISSING ITEMS:
{chr(10).join(['[ ] ' + s for s in score.get('missing_actions', [])])}
"""
            ...
            docs_cursor = self.db.documents.find({"case_id": case_id})
