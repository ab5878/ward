...
class SimilarityEngine:
    ...
    async def find_similar_cases(self, current_case: Dict, limit: int = 3) -> List[Dict[str, Any]]:
        ...
        cursor = self.db.cases.find({
            "disruption_details.disruption_type": disruption_type,
            "status": "RESOLVED",
            "_id": {"$ne": current_case["_id"]}
        }).sort("created_at", -1).limit(20) # Fetch last 20 candidates
        ...
        chat = LlmChat(
            ...
            session_id=f"similarity-{current_case.get('_id')}",
